...
const astToTS = ( ast: Node | List<Node> | SingleProp | string | boolean ): string => {
	...
	if ( ast instanceof Call )
		return `${ ast.name}(${ ast.args ? ` ${[ ...ast.args ].map( astToTS ).join( ", " )} ` : ""})`;
	...
	if ( ast instanceof EmptyLine )
		return "";

	if ( ast instanceof ExitWhen )
		return `if ( ${astToTS( ast.data[ 0 ] )} ) break;`;
	...
	if ( ast instanceof JASSFunction ) {
		...
		if ( ast.returns === undefined )
			isVoid = true;

		const bodyContent = ast.statements ? astToTS( ast.statements ) : "";
		const body = bodyContent ? `\n\n${bodyContent}\n\n` : "";
		const r = `const ${ ast.name} = (${ ast.params ? ` ${[ ...ast.params ].map( astToTS ).join( ", " )} ` : ""}): ${jassTypeToTypeScriptType( ast.returns || "void" )} => {${body}};\n`;
		isVoid = false;
		return r;
	...
	if ( ast instanceof Loop )
		return `\nwhile ( true ) {\n\n${[ ...ast.statements ].map( astToTS ).join( "\n" ).split( "\n" ).map( v => v ? "\t" + v : v ).join( "\n" )}\n\n}\n\n`;

	if ( ast instanceof Name )
		return ast.replace( "this", "_this" );

	if ( ast instanceof Native )
		if ( ast instanceof Type )
			return "";

	if ( ast instanceof Param )
		return `${ ast.name.replace( "this", "_this" )}: ${jassTypeToTypeScriptType( ast.type )}`;

	if ( ast instanceof Parens )
		return `( ${astToTS( ast.data[ 0 ] )} )`;

	if ( ast instanceof Program )
		return [ ...ast ].map( astToTS ).join( "\n" );

	if ( ast instanceof Return )
		if ( ! isVoid )
			return `return ${astToTS( ast.data[ 0 ] )};`;
		else
			return "return;";
